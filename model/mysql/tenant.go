...
type Tenant struct {
	...
	AllowedPlatforms datatypes.JSON  `gorm:"column:allowed_platforms;type:json;comment:'允许的平台'"  json:"allowed_platforms"`                                   // 新增
...
// GetAllowedPlatforms 解析租户允许的平台列表
func (t *Tenant) GetAllowedPlatforms() []constant.PlatformType {
	var platforms []constant.PlatformType
	_ = json.Unmarshal(t.AllowedPlatforms, &platforms)
	return platforms
}

// IsPlatformAllowed 校验平台是否在租户允许范围内
func (t *Tenant) IsPlatformAllowed(p constant.PlatformType) bool {
	for _, allowed := range t.GetAllowedPlatforms() {
		if allowed == p {
			return true
		}
	}
